...
Piece Name: {gunTarget.name}";
        ...
        private static void SetHoverAllowed(bool allowed)
        {
            GTPlayer player = GTPlayer.Instance;
            if (player == null)
                return;

            if (!hoverAllowedLookupDone)
            {
                hoverAllowedLookupDone = true;

                PropertyInfo property = typeof(GTPlayer).GetProperty("isHoverAllowed", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                if (property?.CanWrite == true)
                {
                    hoverAllowedMember = property;
                }
                else
                {
                    MethodInfo method = typeof(GTPlayer).GetMethod("SetHoverAllowed", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                    if (method != null)
                        hoverAllowedMember = method;
                }

                if (hoverAllowedMember == null)
                    LogManager.LogError("GTPlayer hover API not found (isHoverAllowed / SetHoverAllowed) — hoverboard mods cannot activate hover.");
            }

            try
            {
                switch (hoverAllowedMember)
                {
                    case PropertyInfo property:
                        property.SetValue(player, allowed);
                        break;
                    case MethodInfo method:
                    {
                        ...
                        {
                            object fallback = parameters[1].HasDefaultValue
                                ? parameters[1].DefaultValue
                                : Activator.CreateInstance(parameters[1].ParameterType);
                            args = new[] { (object)allowed, fallback };
                        }
                        else
                            args = new[] { (object)allowed };
            ...
            {
                LogManager.LogError($"Failed to set hover allowed: {ex.Message}");
            }
        }
        ...
        public static void SendThrowableProjectile(int index, Vector3 pos, Vector3 vel, Quaternion rot)
        {
            ...
            if (Time.time > throwableProjectileTimeout)
            {
                ...
                SerializeWritePatch.positionOverride = pos;
                try
                {
