...
namespace iiMenu.Mods
{
    public static class Safety
    {
        ...
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isOfflineVRRig && vrrig.CosmeticsString().Contains("LBAAK") || vrrig.CosmeticsString().Contains("LBAAD") || vrrig.CosmeticsString().Contains("LMAPY")))
            {
                try
                {
        ...
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isOfflineVRRig && Visuals.specialCosmetics.Keys.Any(x => vrrig.CosmeticsString().Contains(x))))
            {
                try
                {
        ...
        public static void CosmeticNotifications()
        {
            ...
            {
                foreach (var cosmetic in Visuals.specialCosmetics.Where(cosmetic => rig.CosmeticsString().Contains(cosmetic.Key)))
                {
                    specialRig = rig;
