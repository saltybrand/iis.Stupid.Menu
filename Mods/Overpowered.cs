...
namespace iiMenu.Mods
{
    public static class Overpowered
    {
        ...
        public static IEnumerator DisableSnowball(bool rigDisabled)
        {
            ...
            foreach (SnowballThrowable snowball in snowballDict?.Values ?? Enumerable.Empty<SnowballThrowable>())
            {
                if (snowball == null)
                    continue;
        ...
        {
            if (!PhotonNetwork.InRoom || GorillaTagger.Instance == null || GorillaTagger.Instance.myVRRig == null || CoroutineManager.instance == null)
                yield break;
        ...
        public static void BetaSpawnSnowball(Vector3 Pos, Vector3 Vel, int Mode, Player Target = null, int? customScale = null, bool ignoreMultiply = false)
        {
            ...
            try
            {
                if (!PhotonNetwork.InRoom || GorillaTagger.Instance == null || GorillaTagger.Instance.bodyCollider == null || GorillaTagger.Instance.myVRRig == null)
                    return;
                ...
                Vector3 archivePosition = VRRig.LocalRig != null ? VRRig.LocalRig.transform.position : Vector3.zero;
                bool isTooFar = VRRig.LocalRig != null && Vector3.Distance(Pos, GorillaTagger.Instance.bodyCollider.transform.position) > 3.9f;
                if (isTooFar)
                {
                ...
                for (int i = 0; i < (ignoreMultiply ? 1 : snowballMultiplicationFactor); i++)
                {
                    ...
                    if (nextGrowingSnowball != null)
                        nextGrowingSnowball.SetSnowballActiveLocal(true);
                }

                if (NoTeleportSnowballs && isTooFar && VRRig.LocalRig != null)
                {
            ...
            if (rightGrab)
            {
                ...
                else
                {
                    ...
                    if (Vector3.Distance(critter.transform.position, GorillaTagger.Instance.bodyCollider.transform.position) > 25f)
                    {
                        ...
                        ServerPos = critter.transform.position - Vector3.one * 5f;
                    }
        ...
        public static void CritterGun()
        {
            if (GetGunInput(false))
            {
                ...
                if (GetGunInput(true))
                {
                    if (CrittersManager.instance == null)
                        return;
